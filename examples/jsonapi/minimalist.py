...
from schema import JsonApificator
...
@requests_post.post(schema=PostSchema.orig, validators=(marshmallow_body_validator,), response_schemas=[PostSchema])
def _requests_post(request):
    ...
    :return:
    """
    return PostSchema().dump(Post(request.validated['uuid'], request.validated['title']))


@requests_get.get()
def _requests_get(request):

    return PostSchema().dump(Post(1, 'Title'))
    # return {'title': 'Tomas'}


blog_post = Service(name='blog_post', path='/api/v1/blog', tags=['blog'])


@blog_post.post(schema=BlogSchema, validators=(marshmallow_body_validator,))
def _requests_post(request):
    """
