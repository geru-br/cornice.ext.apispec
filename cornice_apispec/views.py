...
def swagger_ui_template_view(request):
    ...
    `cornice_apispec.swagger_ui_script_generator` in pyramid config, it defaults
    to 'cornice_apispec.views:swagger_ui_script_template'

    :param request:
    :return:
    """
    script_generator = request.registry.settings.get(
        'cornice_apispec.swagger_ui_script_generator',
        'cornice_apispec.views:swagger_ui_script_template')
    package, callable = script_generator.split(':')
    imported_package = importlib.import_module(package)
    script_callable = getattr(imported_package, callable)
    template = pkg_resources.resource_string(
        'cornice_apispec', 'templates/index.html').decode('utf8')
...
def open_api_json_view(request):
    ...
    """
    doc = cornice_apispec.CorniceSwagger(
        cornice.service.get_services(), pyramid_registry=request.registry)
    kwargs = request.registry.settings['cornice_apispec.spec_kwargs']
...
def swagger_ui_script_template(request, **kwargs):
    ...
    into index template
    """
    swagger_spec_url = request.route_url('cornice_apispec.open_api_path')
    template = pkg_resources.resource_string(
        'cornice_apispec',
